//! LUT-based 7-card evaluator with packed u32 score.
//! Suits: 0..3, Ranks: 0..12 (2..A).
//! Hand is [u16;4] bitmasks (13 bits used per suit).
...
// Keep original HIBIT13 - the packing made things worse
pub static HIBIT13: Lazy<[i8; 8192]> = Lazy::new(|| {
    let mut t = [0i8; 8192];
    t[0] = -1;
    let mut m = 1usize;
    while m < 8192 {
        t[m] = (15 - (m as u16).leading_zeros() as i32) as i8;
        m += 1;
    }
    t
});
...
pub fn top5_from_mask(mut m: u16) -> (i32, i32, i32, i32, i32) {
    let r0 = msb_index(m);
    if r0 < 0 { return (-1, -1, -1, -1, -1); }
    m &= !(1u16 << r0);
    
    let r1 = msb_index(m);
    if r1 < 0 { return (r0, -1, -1, -1, -1); }
    m &= !(1u16 << r1);
    
    let r2 = msb_index(m);
    if r2 < 0 { return (r0, r1, -1, -1, -1); }
    m &= !(1u16 << r2);
    
    let r3 = msb_index(m);
    if r3 < 0 { return (r0, r1, r2, -1, -1); }
    m &= !(1u16 << r3);
...
pub fn evaluate_u32(hand: &Hand) -> u32 {
    ...
    // Straight flush - keep original but compute all checks first
    let pc0 = popcnt13(h0);
    let pc1 = popcnt13(h1);
    let pc2 = popcnt13(h2);
    let pc3 = popcnt13(h3);
    ...
    // Flush - using the already computed popcounts
    let mut m: u16 = 0;
    if pc0 >= 5 { m = h0; }
    else if pc1 >= 5 { m = h1; }
    else if pc2 >= 5 { m = h2; }
